...
`go-ruby-fast-gettext/fast-gettext` reimplements Ruby's `fast-gettext` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-fast-gettext/fast-gettext` directly, with no Ruby runtime;
...
Because `fast-gettext` is pure Go with cgo disabled, it also compiles to **WebAssembly**
